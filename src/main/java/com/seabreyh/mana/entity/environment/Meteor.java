...
public class Meteor extends AbstractArrow implements SpawnPredicate {
    private int age;
    ...
    public static final ResourceLocation METEORITE_LOOT_TABLE = new ResourceLocation(ManaMod.MOD_ID,
            "chests/meteorite");

    public Meteor(EntityType<? extends Meteor> getEntity, Level world) {
        super(getEntity, world);
        this.isFalling = true;
        this.dimensions = getEntity.getDimensions();
    }

    public Meteor(EntityType<? extends Meteor> getEntity, Level world, Player ownPlayer) {
    ...
    protected void onHitBlock(BlockHitResult hitResult) {
        if (!this.level.isClientSide()) {
            this.level.explode((Entity) null, this.getX(), this.getY(), this.getZ(), 6.0F,
                    Explosion.BlockInteraction.DESTROY);

            BlockPos pos = hitResult.getBlockPos();

            float radius = 10.0f;
            BlockPos positiveRadiusPosition = pos.offset(radius, radius, radius); // Gets one corner of the cube in the
            // positive X, Y, and Z direction
            BlockPos negativeRadiusPosition = pos.offset(-1 * radius, -1 * radius, -1 * radius); // Gets the opposite
                                                                                                 // corner

            // create random meteor debris (magma)
            Iterable<BlockPos> cubeResult = BlockPos.randomInCube(this.level.random, 50000, pos, 20);
            for (BlockPos targetPos : cubeResult) {
                if (targetPos.distSqr(pos) < 50) {
                    if (!(this.level.getBlockState(targetPos).getBlock() instanceof LiquidBlock)
                            && !(this.level.getBlockState(targetPos).getBlock() instanceof AirBlock)) {
                        this.level.setBlockAndUpdate(targetPos, Blocks.MAGMA_BLOCK.defaultBlockState());
                    }
                }
            }

            // create random meteor debris (obsidian)
            cubeResult = BlockPos.randomInCube(this.level.random, 50000, pos, 20);
            for (BlockPos targetPos : cubeResult) {
                if (targetPos.distSqr(pos) < 50) {
                    if (!(this.level.getBlockState(targetPos).getBlock() instanceof LiquidBlock)
                            && !(this.level.getBlockState(targetPos).getBlock() instanceof AirBlock)) {
            ...
            // Create loot chest
            pos = pos.offset(0, -3, 0);
            this.level.setBlockAndUpdate(pos, Blocks.CHEST.defaultBlockState());
            ChestBlockEntity cBlockEntity = new ChestBlockEntity(pos, Blocks.CHEST.defaultBlockState());
            this.level.setBlockAndUpdate(pos.offset(1, 0, 0),
                    Blocks.GILDED_BLACKSTONE.defaultBlockState());
            this.level.setBlockAndUpdate(pos.offset(-1, 0, 0),
                    Blocks.GILDED_BLACKSTONE.defaultBlockState());
            this.level.setBlockAndUpdate(pos.offset(0, 1, 0),
                    Blocks.GILDED_BLACKSTONE.defaultBlockState());
            this.level.setBlockAndUpdate(pos.offset(0, -1, 0),
                    Blocks.GILDED_BLACKSTONE.defaultBlockState());
            this.level.setBlockAndUpdate(pos.offset(0, 0, 1),
                    Blocks.GILDED_BLACKSTONE.defaultBlockState());
            this.level.setBlockAndUpdate(pos.offset(0, 0, -1),
                    Blocks.GILDED_BLACKSTONE.defaultBlockState());
            cBlockEntity.setLootTable(METEORITE_LOOT_TABLE, 0);
            this.level.setBlockEntity(cBlockEntity);
    ...
    private void playHitSound() {
        this.level.playSound((Player) null, this.getX(), this.getY(), this.getZ(), HIT_SOUND, SoundSource.AMBIENT, 3.5F,
