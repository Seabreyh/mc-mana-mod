...
public class FallenStarOld extends ThrowableItemProjectile {
    private static final EntityDataAccessor<Byte> ID_FLAGS = SynchedEntityData.defineId(Arrow.class,
    ...
    public FallenStarOld.Pickup pickup = FallenStarOld.Pickup.DISALLOWED;
    ...
    public FallenStarOld(EntityType<? extends FallenStarOld> p_36721_, Level p_36722_) {
        super(p_36721_, p_36722_);
        this.bobOffs = this.random.nextFloat() * (float) Math.PI * 2.0F;
    }

    protected FallenStarOld(EntityType<? extends FallenStarOld> p_36711_, double p_36712_, double p_36713_,
            double p_36714_,
            Level p_36715_) {
    ...
    protected FallenStarOld(EntityType<? extends FallenStarOld> p_36717_, LivingEntity p_36718_, Level p_36719_) {
            ...
            if (this.isCritArrow()) {
                for (int i = 0; i < 4; ++i) {
                    this.level.addParticle(ParticleTypes.CRIT, this.getX() + d5 * (double) i / 4.0D,
                            this.getY() + d6 * (double) i / 4.0D, this.getZ() + d1 * (double) i / 4.0D, -d5, -d6 + 0.2D,
                            -d1);
                }
            }
    ...
    protected boolean canHitEntity(Entity p_36743_) {
        return super.canHitEntity(p_36743_)
                && (this.piercingIgnoreEntityIds == null || !this.piercingIgnoreEntityIds.contains(p_36743_.getId()));
    }
    ...
    public void readAdditionalSaveData(CompoundTag p_36761_) {
        ...
        this.pickup = FallenStarOld.Pickup.byOrdinal(p_36761_.getByte("pickup"));
        this.setCritArrow(p_36761_.getBoolean("crit"));
        if (p_36761_.contains("SoundEvent", 8)) {
            this.soundEvent = Registry.SOUND_EVENT.getOptional(new ResourceLocation(p_36761_.getString("SoundEvent")))
    ...
    public void setOwner(@Nullable Entity p_36770_) {
        ...
        if (p_36770_ instanceof Player) {
            this.pickup = ((Player) p_36770_).getAbilities().instabuild ? FallenStarOld.Pickup.CREATIVE_ONLY
                    : FallenStarOld.Pickup.ALLOWED;
        }
    ...
    public void setEnchantmentEffectsFromEntity(LivingEntity p_36746_, float p_36747_) {
        ...
        this.setBaseDamage((double) (p_36747_ * 2.0F) + this.random.nextGaussian() * 0.25D
                + (double) ((float) this.level.getDifficulty().getId() * 0.11F));
        if (i > 0) {
    ...
    public static enum Pickup {
        ...
        public static FallenStarOld.Pickup byOrdinal(int p_36809_) {
