...
public class DamageTypeTagGen extends TagsProvider<DamageType> {
    ...
    public DamageTypeTagGen(PackOutput pOutput, CompletableFuture<HolderLookup.Provider> pLookupProvider,
        ...
        this(ManaMod.MOD_ID, pOutput, pLookupProvider, existingFileHelper);
    }
    ...
    protected void addTags(@NotNull Provider provider) {
        ...
        tagsToTypes.asMap()
                .forEach((tag, keys) -> {
                    ...
                    keys.forEach(key -> appender.addOptional(key.location())); // FIXME: 1.20 usind add() results in
                                                                               // datagen failure, is this approach
                                                                               // correct?
                });
    }
}